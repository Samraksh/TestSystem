...
namespace TestRig
{
    public class TestStatus
    {
        ...
        public void TestDurationTimerCallback(Object stateInfo)
        {
            //logic.stopMeasure();
        }
        ...
        {
            TimerCallback tcb = TestDurationTimerCallback;

            while (true)
            {
                // if there are no tests in the system, busy wait
                while (_testCollection.Count == 0 || TestRig.Window1.synchronize == false) ;
                ...
                Timer TestTimer = new Timer(tcb, null, 0, 20000);

                logic.startMeasure(PowerShell.outputFileName);
